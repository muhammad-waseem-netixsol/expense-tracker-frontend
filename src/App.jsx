...
function App() {
  ...
  useEffect(()=> {
    const transactionsFetchHandler = async () => {
      setLoading(true);
      const transactions = await fetch("https://expense-tracker-backend-smoky.vercel.app/transactions");
      if(!transactions.ok){
        alert("Error fetching transactions");
      }
      const resposne = await transactions.json();
      setExpenses(resposne.expenses);
      setLoading(false);
    };
    transactionsFetchHandler();
  }, [added])
  const addTransactionHandler = () => {
    ...
    transactionAddHandler(obj);
 
  setAmount("");
  setText("");
  };
  const transactionAddHandler = async (obj) => {
   
    const tranxtion = await fetch("https://expense-tracker-backend-smoky.vercel.app/transaction", {
      method: "POST", 
      headers: {
   ...
   const res = await tranxtion.json()
  ...
  return (<>
    ...
    <div className="max-w-[400px] mx-auto">
      ...
      <div>
       ...
       {!loading && <div> {expenses.length === 0 && <h1 className="font-medium text-center py-10">No transaction yet!</h1>}
        {expenses.length > 0 && expenses.map((exp, i) => <Card deleteDone={deleteHandler} expenses={exp} key={expenses[i].id} />)}</div>
   }
   {loading && <div className="w-full flex items-center justify-center"><div className="lds-ripple"><div></div><div></div></div></div>}
      <div className="py-2">
